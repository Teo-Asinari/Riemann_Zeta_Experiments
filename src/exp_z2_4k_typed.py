...
def exp_1_over_z2_numpy(
    z: ComplexArray, exclusion_radius: float = 0.01
) -> ComplexArray:
    ...
    result_mag_capped = np.where(
        ~np.isfinite(result_mag_capped), 1e6, result_mag_capped
    )
...
if NUMBA_AVAILABLE:
    ...
    def exp_1_over_z2_jit(
        z_real: RealArray, z_imag: RealArray
...
def create_4k_visualization(
    ...
    print(
        f"Creating 4K visualization "
        f"({resolution}x{resolution} = {resolution**2:,} points)"
    )
    print(f"Domain: {xlim[0]} to {xlim[1]} (real), " f"{ylim[0]} to {ylim[1]} (imag)")
    print(f"Exclusion zone: radius = {exclusion_radius} " f"(black circle in center)")
    ...
    print(
        f"Function value range: min={np.nanmin(np.abs(W)):.2e}, "
    ...
    print(f"RGB range: min={np.nanmin(rgb):.3f}, " f"max={np.nanmax(rgb):.3f}")
    print(
        f"Brightness range: min={np.nanmin(brightness):.3f}, "
...
def main() -> None:
    ...
    try:
        ...
        print(
            "The image reveals incredible fractal detail "
